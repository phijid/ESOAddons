...
function CombatAlerts.CombatEvent( eventCode, result, isError, abilityName, abilityGraphic, abilityActionSlotType, sourceName, sourceType, targetName, targetType, hitValue, powerType, damageType, log, sourceUnitId, targetUnitId, abilityId )
	...
	elseif (result == ACTION_RESULT_BEGIN and CombatAlertsData.u38.phobic[abilityId]) then
	...
	-- Scions of Ithelia -------------------------------------------------------
	elseif (result == ACTION_RESULT_BEGIN and CombatAlertsData.u41.banners[abilityId]) then
		CombatAlerts.Alert(nil, GetFormattedAbilityName(abilityId), CombatAlertsData.u41.banners[abilityId], nil, 1500)
	elseif (CombatAlertsData.damageEvents[result] and targetType == COMBAT_UNIT_TYPE_PLAYER and CombatAlertsData.u41.standingInAoe[abilityId]) then
		local params = CombatAlertsData.u41.standingInAoe[abilityId]
		if (not (params[2] and CombatAlerts.isTank)) then
			CombatAlerts.AlertBorder(true, params[1])
		end
	elseif (result == ACTION_RESULT_EFFECT_GAINED_DURATION and CombatAlertsData.u41.targeted[abilityId]) then
		if (CombatAlerts.units[targetUnitId]) then
			targetName = CombatAlerts.units[targetUnitId].name
		end
		CombatAlerts.Alert(GetFormattedAbilityName(abilityId), targetName, 0xCC3399FF, SOUNDS.CHAMPION_POINTS_COMMITTED, 2000)
	elseif (result == ACTION_RESULT_EFFECT_GAINED_DURATION and abilityId == CombatAlertsData.u41.poison and targetType == COMBAT_UNIT_TYPE_PLAYER) then
		CombatAlerts.Alert(nil, GetFormattedAbilityName(abilityId), 0x00CC00FF, SOUNDS.CHAMPION_POINTS_COMMITTED, 2500)
	elseif (result == ACTION_RESULT_EFFECT_GAINED_DURATION and abilityId == CombatAlertsData.u41.webspinner.spawn) then
		CombatAlerts.Alert(nil, GetFormattedAbilityName(CombatAlertsData.u41.webspinner.name), 0xFFFFFFFF, SOUNDS.OBJECTIVE_DISCOVERED, 1500)
	elseif (CombatAlerts.isVet and result == ACTION_RESULT_EFFECT_GAINED_DURATION and abilityId == CombatAlertsData.u41.impale.start) then
		local currentTime = GetGameTimeMilliseconds()
		if (currentTime - CombatAlerts.u41.lastImpale > 2000) then
			CombatAlerts.u41.lastImpale = currentTime
			CombatAlerts.CastAlertsStart(CombatAlertsData.u41.impale.damage, GetFormattedAbilityName(CombatAlertsData.u41.impale.damage), hitValue, CombatAlertsData.u41.impale.timer, nil, { hitValue, "Avoid Lines!", 1, 0.4, 0, 0.5, nil })
		end
	end
end
